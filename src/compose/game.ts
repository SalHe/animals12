...
function initCells(): [Ref<Cell[]>, Cell[], Cell[]] {
    ...
    // 初始化格子并发放棋子
    const cells = ref(Array.from({ length: cols * rows }).map((_, i): Cell => {
        const y = Math.floor(i / cols);
        const x = i % cols;
        const getChess = () => initChesses.value.splice(0, 1)[0];
...
export function useBoard(gameOver: (winner?: string) => void): [Cell[], Ref<string | undefined>, (cell: Cell) => void] {
    ...
    const clickCell = (cell: Cell): void => {
        ...
        if (!selectedCell.chess) { // 实际上到这里，这一种情况不会发生，只是为了过静态语法检查，懒得写 !.
            return;
        }
        ...
        // 取消选中/选中另一个棋子 可以自残
        if (selectedCell === cell || (cell.chess.player === turn.value && (!canAttack(selectedCell.chess.id, cell.chess.id) || !canGoto(selectedCell, cell)))) {
...
export function useRestartableBoard(gameOver: (winner?: string) => void): [Ref<Cell[]>, Ref<string | undefined>, (cell: Cell) => void, () => void] {
    const [cells, turn, clickCell] = useBoard(gameOver);
