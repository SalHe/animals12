...
export function canGoto(cell1: Cell, cell2: Cell, bankOccupier?: string): boolean {
    if (
        ...
        // 龙、象之外的动物不允许下河
        || cell2.cellType === "river" && cell1.chess?.id !== Dragon && cell1.chess?.id !== Elephant && cell1.chess?.id !== Leopard

        // 河里不能游动
        || cell1.cellType === "river" && cell2.cellType === "river"

        // 如果对方已堵岸，则不能从河里上陆地
        || cell2.cellType === "river" && cell1.cellType === "normal" && bankOccupier && cell1.chess?.player !== bankOccupier
    ) return false;
